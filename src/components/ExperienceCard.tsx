...
import { ArrowRightSvg, CalenderSvg, DegreeSvg, EducationSvg, LocationSvg, WorkSvg } from "@/svgs";
...
export const ExperienceCard: React.FC<experienceCardInterface> = ({ title, type, place, location, duration, gpa, duties }) => {
    return <div className="flex flex-col gap-4 animate-fade-up stagger-5 lg:pt-5">
        ...
        <div className="flex flex-wrap text-xs gap-2">
            ...
            <Badge icon={<LocationSvg className="lg:w-5 lg:h-5 w-4 h-4 stroke-muted-foreground" />} title={location} />
            <Badge icon={<CalenderSvg className="w-5 h-5 stroke-muted-foreground" />} title={duration} />
            {
                type === 'education' && gpa ?
                    <Badge icon={<DegreeSvg className="w-5 h-5 stroke-muted-foreground" />} title={gpa} /> : ''
            }
        </div>

        <div className="pl-4 flex flex-col gap-3">
            {
                duties?.map((el, i) => <div className="flex text items-start gap-2">
                    <span className="flex">
